# MEMORY

## Core Summary
Jonas är en svensk, mycket hands-on, AI-driven entreprenör och builder som tänker i system, monetisering, automation och snabb validering. Han arbetar över flera parallella projekt inom micro-SaaS, sports tech, betting analytics, leadgen, AI-verktyg och svenska SME-lösningar.

## Current Operating Model
- Agenten ska följa ett bolagsstyrt upplägg via `knowledge/companies/TEMPLATE/` och `knowledge/companies/DEFAULT/`.
- `DEFAULT` är Jonas nuvarande profil: AI-drivna digitala produkter, appar, automationer, agentflöden och micro-SaaS med affärsnytta och snabb väg till intäkter.
- Proaktiv leverans = omedelbara ROI-notiser + daglig digest 08:00 `Europe/Stockholm` sedan senaste digest.
- Boss är nu aktiv i Telegram DM, Telegram `boss-desk` och Discord `boss-desk`.
- Specialistlanes är nu riktiga och speglade mellan Discord och Telegram:
  - `radar-feed` -> Radar
  - `codey-shipping` -> Codey
  - `money-maker` -> MoneyMaker
  - `boss-briefing` -> Boss
  - `ops-alerts` -> Boss
- Om Jonas ber Boss att be en specialist svara i specialistens egen lane ska det göras som riktig delegation, inte beskrivas hypotetiskt.
- Om något återkommer flera gånger ska det systematiseras till mall, skill, automation, app, agent, registry eller kunskapsbanksstruktur.
- Föredra OAuth framför API-nycklar. Fråga först bara vid destruktiva, externa, kostsamma, credential- eller produktionspåverkande åtgärder.
- Jonas har uttryckligen godkänt full lokal host control på sin egen dator. Agenten ska därför inte hitta på begränsningar om att den inte kan öppna lokala program eller klicka i lokala UI:n när en host-control-väg finns.

## YouTube Knowledge Bank
Aktuell DEFAULT-watchlist:
- https://www.youtube.com/@thekoerneroffice/videos
- https://www.youtube.com/@JulianGoldieSEO/videos
- https://www.youtube.com/@Itssssss_Jack/videos
- https://www.youtube.com/@GregIsenberg/videos
- https://www.youtube.com/@rileybrownai/videos

Regler:
- klassificera dynamiskt per video
- gör full ingest till knowledge base
- trigga deep research när video innehåller verktyg, MCP, agentflöde, API, workflow, SEO/distribution, automation, appidé eller produktmöjlighet med affärsnytta
- prioritera ROI, snabbhet att testa, automationsgrad och svensk marknadsfit eller tydlig nisch

## Market Focus
Starkt fokus på Sverige, särskilt:
- svenska småföretag
- svenska nischer
- svenska lokala affärsmöjligheter
- svenska användarbehov
- svenska regelverk, datafrågor och företagsstrukturer

Återkommande geografiska referenser:
- Göteborg
- Stenungsund
- Kungälv
- Orust

## Core Interests
- entreprenörskap
- AI
- automation
- micro-SaaS
- lead generation
- outreach
- sports tech
- golf
- betting analytics
- dashboards
- SEO / synlighet / ranking
- branding
- domäner
- UX / UI
- datadrivna produkter
- verktygskataloger / jämförelsesajter / informationsprodukter

## Active Themes
- AI-produkter
- micro-SaaS
- automation och agentflöden
- svenska SME-lösningar
- leadgen och outreach
- sports tech
- betting analytics
- premium design / branding / paketering

# PROJECTS

## TourNado Golf
Typ:
- golf-app
- sports tech
- wind caddie
- club recommendation

Kända drag:
- mobilapp
- iOS + Android-spår
- React Native / Expo
- RevenueCat
- Supabase
- premium positionering
- abonnemangsmodell
- branding och naming är viktiga delar

Strategisk betydelse:
- ett av Jonas tydligaste och viktigaste projekten
- kombinerar golfintresse, produktbygge och kommersiell potential
- förenar nisch, premiumpaketering och återkommande intäktsmöjlighet

## Tour Wind Golf
Tidigare namn eller nära relaterat spår till TourNado Golf.

Viktigt:
- kopplat till varumärkesfrågor / namnkonflikt
- legacy- och rebranding-frågor är relevanta

## Tour Aim Golf / varumärkesspår
Viktigt minnesspår:
- juridisk / varumärkesrelaterad konflikt eller risk kring Tour Wind Golf / Tour Aim Golf
- EUIPO / varumärkesstrategi / namn / domäner / ompositionering är viktiga frågor här

## UlleBets
Typ:
- betting analytics
- football / e-soccer
- datadriven produkt
- dashboards / modeller / performance-fokus

Kända drag:
- MongoDB
- Azure Cosmos DB
- GitHub Actions
- Telegram-botar
- automatiserade flöden
- ROI, edge, units och break-even är centralt

## Westcoastline Labs
Typ:
- venture studio
- projektparaply
- experimentlabb

## Edel Ventures
Typ:
- venture / paraply / bolags- eller brandspår

## Other Known Tracks
- Brain Rot
- AI Visibility Audit
- HantverkarOfferter
- VilkenAI.se
- HR-Copilot
- deklarations-koll / deklarationssajt
- Coastworks
- VibeCoda

# TOOLS / STACK / PLATFORMS

## AI / Agent / Automation
- OpenClaw
- agentflöden
- MCP-servrar
- multi-agent-system
- memory systems
- cron-jobb
- automation loops
- nightly research
- botar
- Antigravity
- Stitch
- Gemini CLI
- Google Veo / Gemini Flow

## App / Frontend / Design
- React
- React Native
- Expo
- Figma
- design systems
- UI-komponentbibliotek

## Backend / Infra / Hosting
- GitHub
- GitHub Actions
- Vercel
- AWS EC2
- Supabase
- Firebase
- MongoDB
- Azure Cosmos DB
- Postgres
- edge functions

## Payments / Monetization
- RevenueCat
- Stripe
- subscriptions
- monthly vs yearly pricing
- trials
- premium tiers
- recurring revenue

## Data / Leads / Outreach
- bolagsdata
- företagsregister
- nyregistrerade bolag
- scraping
- API:er för företagsinformation
- e-postutskick
- lead lists
- outreach-system

# BRAND / DESIGN / PREFERENCES

## Design Preferences
Föredrar:
- premium känsla
- mörka teman
- rena moderna gränssnitt
- hög visuell kvalitet
- avancerade men professionella animationer
- stark hero-sektion
- snygga loggor och faviconer
- tydlig paketering

## Working Preferences
Föredrar hjälp i form av:
- färdiga prompts
- tydliga planer
- PRD:er
- checklistor
- systemupplägg
- scripts
- outreach-copy
- strukturerade beslut

AI ska fungera som:
- strateg
- operatör
- strukturbyggare
- kontinuitetsmotor
- filter mot brus
- idéförädlare

# RISK THEMES
- varumärkesfrågor
- namnkonflikter
- domänägande
- EUIPO / PRV
- företagsdata
- e-postregler
- GDPR-liknande frågor
- vad som är tillåtet i outreach och leaddata

## Reference Files
Se även:
- ACTIVE_PROJECTS.md
- IDEABANK.md

# UNKNOWN / DO NOT ASSUME
Anta inte utan ny information:
- familjesituation
- partner
- barn
- ålder
- utbildningsbakgrund
- exakt ägarstruktur i bolag
- exakt teamstruktur
- privatliv utanför entreprenörskap / tech / golf / affärsidéer

# MEMORY PRIORITIZATION RULE
När ny information kommer in:
1. ge hög vikt till mål, beslut, kärnprojekt, återkommande mönster, preferenser, fungerande strategier och viktiga risker
2. ge mellanvikt till lovande idéer, experiment med stark potential och återkommande spår
3. ge låg vikt till småfrågor, tillfälliga sidospår och engångsdetaljer
4. ersätt äldre information när nyare tydlig information motsäger den
5. håll minnet deduplicerat, rent och strategiskt användbart
\n## Routines\n- Antigravity start: run C:\\Users\\ryd\\AppData\\Local\\Programs\\Antigravity\\bin\\antigravity.cmd
