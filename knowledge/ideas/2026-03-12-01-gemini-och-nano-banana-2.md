# Research Memo - gemini och nano banana 2

- Cluster: gemini
- Opportunity score: 57
- Decision: project_upgrade - Passar ett av dina nuvarande projekt starkt och bor omsattas dar.

## Why now
Passar ett av dina nuvarande projekt starkt och bor omsattas dar.

## Best idea to test
- Applicera gemini och nano banana 2 i Coastworks
- Why: Temat overlappar med Coastworks via seo-content, openclaw, knowledge-systems, automation och agent-automation.
- Impact / effort / speed: hog / hog / snabb

## Best project matches
- Coastworks (score 33): seo-content, openclaw, knowledge-systems, automation och agent-automation
- AI Visibility Audit (score 26): seo-content, workflow-automation och automation
- Gravity Claw (score 24): openclaw, knowledge-systems och agent-automation

## Source evidence
- Google Gemini New FREE Updates Are INSANE! (Julian Goldie SEO)
  - Rollfördelningen är tydlig: Notebook LM står för research och struktur, medan Nano Banana 2 driver bildgenereringen och den visuella stilen.
  - De starkaste use casen är utbildning och onboarding och marketing och explainers, vilket gör att samma upplägg kan användas både internt och externt.
- NEW Gemini Embedding 2 Update is INSANE! (Julian Goldie SEO)
  - Det här bör ses som ett repeterbart arbetsflöde, inte bara som en enskild feature-demo.
  - Potentialen ligger i att systematisera och automatisera det löpande arbetet snarare än att lösa varje uppgift manuellt.
- Google Gemini New FREE Updates Are INSANE! (Julian Goldie SEO)
  - Videon visar hur Google Gemini FREE Updates kan förvandla dokument och annat källmaterial till färdiga videor med AI-visuals och voiceover utan traditionell produktion.
  - Rollfördelningen är tydlig: Notebook LM står för research och struktur, medan Nano Banana 2 driver bildgenereringen och den visuella stilen.
- NEW Gemini CLI Update is INSANE! 🤯 (Julian Goldie SEO)
  - Det här bör ses som ett repeterbart arbetsflöde, inte bara som en enskild feature-demo.
  - Potentialen ligger i att systematisera och automatisera det löpande arbetet snarare än att lösa varje uppgift manuellt.
- Google Gemini New FREE Updates Are INSANE! (Julian Goldie SEO)
  - Workflowet är tydligt: ladda upp källmaterial och välja visuell stil.
  - De starkaste use casen är utbildning och onboarding och marketing och explainers, vilket gör att samma upplägg kan användas både internt och externt.
- NEW Google Gemini Update is INSANE! (Julian Goldie SEO)
  - Kärnpositioneringen är inte bara ett nytt verktyg utan en AI-assistent som ska kunna bära mer av det löpande operativa arbetet.
  - Ett separat command center eller dashboard är centralt, eftersom det gör agentens arbete synligt, styrbart och praktiskt användbart i vardagen.
